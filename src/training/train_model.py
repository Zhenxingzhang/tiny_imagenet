...
def fc_layer(input_tensor, num_units, layer_name, act=tf.nn.relu, dropout=True):
...
def prepocess_ims(image):
    return tf.reshape(tf.cast(image, tf.float32) / 255., (64, 64, 3))
...
with tf.name_scope('data'):
    ...
    images_batch, labels_batch = read_record_to_queue("/data/tiny_imangenet/tiny_imagenet_data/val.tfrecord", shapes,
                                                      preproc_func=prepocess_ims, num_epochs=None,
...
out_7 = fc_layer(out_6, num_units=128, layer_name='FC_1', dropout=True)
out_8 = fc_layer(out_7, num_units=256, layer_name='FC_2', dropout=True)
...
train_op = tf.train.AdamOptimizer(0.1).minimize(loss_mean)

sess = tf.Session()
summary_op = tf.summary.merge_all()
train_writer = tf.summary.FileWriter('log/tiny_imagenet/9layer/train', sess.graph)
